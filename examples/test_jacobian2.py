...
# phi_p_loop = 1.0
    ...
    sorted_indices_arr = []
    max_num_eqs = 0
    kappa_vals = np.linspace(0.0e9, 20e9, 5)
    ...
    for kappa_ind, kappa_c in enumerate(tqdm(kappa_vals)):
        ...
        dkappa = (kappa_vals[1] - kappa_vals[0])*tau_p
        eq_max, eqs, _ = vcsel.solve_equilibria(nd, guesses=guesses, counts=counts, step=dkappa)
        guesses = []
