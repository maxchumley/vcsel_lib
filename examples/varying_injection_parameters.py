...
kappa_c = np.linspace(30e9,0e9,n_kappa)

final_kappa = np.linspace(0e9,20e9,n_kappa)[448]
...
max_tau_inj_width = 20
n_iterations = 500
for k in range(0, n_iterations):

    kappa_inj_width = 20 * tau          # width (s) — change this to control the Gaussian spread 
    kappa_inj_amp = kappa_c[k]
    ...
    axs[0].set_title(rf'Injection Strength: {kappa_inj_amp*1e-9:.2f} ns$^{{-1}}$', fontsize=24)
    ...
    plt.tight_layout()
    # plt.savefig(f'./{N_lasers}_laser_injection_continuation/{500-k}.png')
    plt.show()
    plt.close(fig)
